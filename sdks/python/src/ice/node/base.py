...
GlobalErrorHandler = Callable[["Node | None", "Roam", Exception], RunState]
...
def set_global_error_handler(handler: GlobalErrorHandler | None) -> None:
    ...
    Args:
        handler: A function that takes (node, roam, exception) and returns RunState.
...
class ErrorHandler(Protocol):
    ...
    Implement the error_handle method in your leaf node to handle errors:
        ...
        class MyLeaf:
            def error_handle(self, roam: Roam, error: Exception) -> RunState:
...
def collect_info(roam: Roam, base: Base, state: str, time_ms: int) -> None:
    """Collect process information for a node execution."""
    if base is None:
        return
...
def collect_reject_info(roam: Roam, base: Base) -> None:
...
def process_with_base(
    base: Base,
    roam: Roam,
    process_node: Callable[[Roam], RunState],
    ...
    try:
        ...
        if base.ice_forward is not None:
            forward_res = base.ice_forward.process(roam)
            if forward_res == RunState.FALSE:
                collect_reject_info(roam, base)
                return RunState.FALSE

            result = process_node(roam)
        ...
        collect_info(roam, base, _state_to_char(result), elapsed)
        ...
        if error_handler is not None and hasattr(error_handler, 'error_handle'):
            error_state = error_handler.error_handle(roam, e)
        elif _global_error_handler is not None:
            # 2. Fall back to global error handler
            error_state = _global_error_handler(None, roam, e)
        ...
        if error_state != RunState.SHUT_DOWN:
            collect_info(roam, base, _state_to_char(error_state), elapsed)
        ...
        # SHUT_DOWN - re-raise
        collect_info(roam, base, "S", elapsed)
        raise
