package context

import (
	"sync"
	"testing"
)

func TestRoam_PutAndGet(t *testing.T) {
	roam := NewRoam()

	// Test Put and Get
	roam.Put("name", "test")
	roam.Put("age", 25)
	roam.Put("score", 85.5)
	roam.Put("active", true)

	if roam.Value("name").String() != "test" {
		t.Error("expected name=test")
	}
	if roam.Value("age").IntOr(0) != 25 {
		t.Error("expected age=25")
	}
	if roam.Value("score").Float64Or(0) != 85.5 {
		t.Error("expected score=85.5")
	}
	if !roam.Value("active").Bool() {
		t.Error("expected active=true")
	}
}

func TestRoam_NilHandling(t *testing.T) {
	roam := NewRoam()

	// Empty key should not be stored
	roam.Put("", "value")
	if roam.Get("") != nil {
		t.Error("empty key should return nil")
	}

	// Nil value should be stored
	roam.Put("key", "value")
	if roam.Get("key") != "value" {
		t.Error("value should be stored")
	}
	roam.Put("key", nil)
	if roam.Get("key") != nil {
		t.Error("nil value should be stored as nil")
	}
	// Verify key exists but value is nil
	if _, exists := roam.Data()["key"]; !exists {
		t.Error("key should exist even with nil value")
	}
}

func TestRoam_Delete(t *testing.T) {
	roam := NewRoam()

	roam.Put("key", "value")
	if roam.Get("key") != "value" {
		t.Error("value should be stored")
	}

	roam.Delete("key")
	if roam.Get("key") != nil {
		t.Error("key should be deleted")
	}
	// Verify key is removed from data
	if _, exists := roam.Data()["key"]; exists {
		t.Error("key should not exist after deletion")
	}

	// Delete non-existent key should not panic
	roam.Delete("nonexistent")

	// Empty key should be ignored
	roam.Delete("")
}

func TestRoam_deepKey(t *testing.T) {
	roam := NewRoam()

	// Test PutDeep
	roam.PutDeep("user.profile.age", 30)
	roam.PutDeep("user.profile.name", "Alice")
	roam.PutDeep("user.score", 100)

	// Test GetDeep
	if roam.GetDeep("user.profile.age") != 30 {
		t.Error("expected user.profile.age=30")
	}
	if roam.GetDeep("user.profile.name") != "Alice" {
		t.Error("expected user.profile.name=Alice")
	}
	if roam.GetDeep("user.score") != 100 {
		t.Error("expected user.score=100")
	}

	// Test non-existent path
	if roam.GetDeep("user.profile.notexist") != nil {
		t.Error("expected nil for non-existent path")
	}
	if roam.GetDeep("notexist.path") != nil {
		t.Error("expected nil for non-existent path")
	}
}

func TestRoam_Resolve(t *testing.T) {
	roam := NewRoam()

	roam.Put("score", 85)
	roam.Put("ref", "@score")
	roam.PutDeep("nested.value", 100) // Use PutDeep for nested keys
	roam.Put("nestedRef", "@nested.value")

	// Test direct value
	if roam.Resolve("direct") != "direct" {
		t.Error("expected direct value")
	}

	// Test reference
	if roam.Resolve(roam.Get("ref")) != 85 {
		t.Error("expected @score to resolve to 85")
	}

	// Test nested reference
	if roam.Resolve(roam.Get("nestedRef")) != 100 {
		t.Error("expected @nested.value to resolve to 100")
	}

	// Test nil
	if roam.Resolve(nil) != nil {
		t.Error("expected nil for nil input")
	}
}

func TestRoam_TypeConversion(t *testing.T) {
	roam := NewRoam()

	// Store various numeric types
	roam.Put("int", 42)
	roam.Put("int64", int64(100))
	roam.Put("float64", 3.14)

	// Int conversions
	if roam.Value("int").IntOr(0) != 42 {
		t.Error("expected int=42")
	}
	if roam.Value("int64").IntOr(0) != 100 {
		t.Error("expected int64 converted to int=100")
	}
	if roam.Value("float64").IntOr(0) != 3 {
		t.Error("expected float64 converted to int=3")
	}
	if roam.Value("notexist").IntOr(-1) != -1 {
		t.Error("expected default value -1")
	}

	// Int64 conversions
	if roam.Value("int").Int64Or(0) != 42 {
		t.Error("expected int converted to int64=42")
	}
	if roam.Value("int64").Int64Or(0) != 100 {
		t.Error("expected int64=100")
	}

	// Float64 conversions
	if roam.Value("int").Float64Or(0) != 42.0 {
		t.Error("expected int converted to float64=42.0")
	}
	if roam.Value("float64").Float64Or(0) != 3.14 {
		t.Error("expected float64=3.14")
	}
}

func TestRoam_ShallowCopy(t *testing.T) {
	roam := NewRoam()
	roam.Put("key1", "value1")
	roam.Put("key2", 100)

	copied := roam.ShallowCopy()

	// Verify copy has same values
	if copied.Value("key1").String() != "value1" {
		t.Error("copy should have key1=value1")
	}
	if copied.Value("key2").IntOr(0) != 100 {
		t.Error("copy should have key2=100")
	}

	// Modify copy should not affect original
	copied.Put("key1", "modified")
	if roam.Value("key1").String() != "value1" {
		t.Error("original should not be modified")
	}
}

func TestRoam_Data(t *testing.T) {
	roam := NewRoam()
	roam.Put("a", 1)
	roam.Put("b", 2)

	data := roam.Data()
	if len(data) != 2 {
		t.Errorf("expected 2 entries, got %d", len(data))
	}
	if data["a"] != 1 || data["b"] != 2 {
		t.Error("data values mismatch")
	}

	// Modify returned data should not affect original
	data["a"] = 999
	if roam.Value("a").IntOr(0) != 1 {
		t.Error("original should not be modified")
	}
}

func TestRoam_ConcurrentAccess(t *testing.T) {
	roam := NewRoam()
	var wg sync.WaitGroup

	// Concurrent writes
	for i := 0; i < 100; i++ {
		wg.Add(1)
		go func(n int) {
			defer wg.Done()
			roam.Put("key", n)
		}(i)
	}

	// Concurrent reads
	for i := 0; i < 100; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
			_ = roam.Get("key")
		}()
	}

	wg.Wait()

	// Should not panic and should have a value
	if roam.Get("key") == nil {
		t.Error("key should have a value after concurrent writes")
	}
}

func TestRoam_NewRoamFrom(t *testing.T) {
	original := map[string]any{
		"name": "test",
		"age":  25,
	}

	roam := NewRoamFrom(original)

	if roam.Value("name").String() != "test" {
		t.Error("expected name=test")
	}
	if roam.Value("age").IntOr(0) != 25 {
		t.Error("expected age=25")
	}

	// Modify original should not affect roam
	original["name"] = "modified"
	if roam.Value("name").String() != "test" {
		t.Error("roam should not be affected by original modification")
	}
}

// Test types for fluent API
type testUser struct {
	Name string
	Age  int
}

func TestRoam_FluentAPI_To(t *testing.T) {
	roam := NewRoam()

	user := &testUser{Name: "Alice", Age: 25}
	roam.Put("user", user)
	roam.Put("name", "Bob")
	roam.Put("count", 100)

	// Test To with pointer type
	var gotUser *testUser
	if !roam.Value("user").To(&gotUser) {
		t.Error("To should return true")
	}
	if gotUser.Name != "Alice" || gotUser.Age != 25 {
		t.Errorf("expected Alice/25, got %s/%d", gotUser.Name, gotUser.Age)
	}

	// Test To with basic types
	var name string
	if !roam.Value("name").To(&name) {
		t.Error("To should return true for string")
	}
	if name != "Bob" {
		t.Errorf("expected Bob, got %s", name)
	}

	var count int
	if !roam.Value("count").To(&count) {
		t.Error("To should return true for int")
	}
	if count != 100 {
		t.Errorf("expected 100, got %d", count)
	}

	// Test To with non-existent key
	var missing string
	if roam.Value("notexist").To(&missing) {
		t.Error("To should return false for non-existent key")
	}

	// Test To with nil dest
	if roam.Value("name").To(nil) {
		t.Error("To should return false for nil dest")
	}
}

func TestRoam_FluentAPI_ValueDeepTo(t *testing.T) {
	roam := NewRoam()

	roam.PutDeep("user.profile.name", "Charlie")
	roam.PutDeep("user.profile.age", 30)

	var name string
	if !roam.ValueDeep("user.profile.name").To(&name) {
		t.Error("ValueDeep To should return true")
	}
	if name != "Charlie" {
		t.Errorf("expected Charlie, got %s", name)
	}
}

func TestRoam_FluentAPI_Value(t *testing.T) {
	roam := NewRoam()

	roam.Put("name", "David")
	roam.Put("age", 28)
	roam.Put("score", 95.5)
	roam.Put("active", true)
	roam.Put("count", int64(1000))

	// Test String
	if roam.Value("name").String() != "David" {
		t.Error("expected name=David")
	}
	if roam.Value("notexist").String() != "" {
		t.Error("expected empty string for non-existent key")
	}
	if roam.Value("notexist").StringOr("default") != "default" {
		t.Error("expected default value")
	}

	// Test Int
	if roam.Value("age").Int() != 28 {
		t.Error("expected age=28")
	}
	if roam.Value("notexist").IntOr(99) != 99 {
		t.Error("expected default value 99")
	}

	// Test Int64
	if roam.Value("count").Int64() != 1000 {
		t.Error("expected count=1000")
	}

	// Test Float64
	if roam.Value("score").Float64() != 95.5 {
		t.Error("expected score=95.5")
	}
	if roam.Value("age").Float64() != 28.0 {
		t.Error("expected int converted to float64=28.0")
	}

	// Test Bool
	if !roam.Value("active").Bool() {
		t.Error("expected active=true")
	}
	if roam.Value("notexist").BoolOr(true) != true {
		t.Error("expected default value true")
	}

	// Test Exists
	if !roam.Value("name").Exists() {
		t.Error("expected name to exist")
	}
	if roam.Value("notexist").Exists() {
		t.Error("expected notexist to not exist")
	}

	// Test Raw
	if roam.Value("name").Raw() != "David" {
		t.Error("expected Raw() to return David")
	}
}

func TestRoam_FluentAPI_ValueDeep(t *testing.T) {
	roam := NewRoam()

	roam.PutDeep("config.server.port", 8080)
	roam.PutDeep("config.server.host", "localhost")

	if roam.ValueDeep("config.server.port").Int() != 8080 {
		t.Error("expected port=8080")
	}
	if roam.ValueDeep("config.server.host").String() != "localhost" {
		t.Error("expected host=localhost")
	}
}
