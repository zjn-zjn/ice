...
public class IceConfServiceImpl implements IceConfService {
    ...
    private void update(IceConf operateConf, long iceId) {
        ...
        // 确保默认值
        if (operateConf.getStatus() == null) {
            operateConf.setStatus(IceStorageConstants.STATUS_ONLINE);
        }
        if (operateConf.getTimeType() == null) {
            operateConf.setTimeType((byte) 1);
        }
        if (operateConf.getDebug() == null) {
            operateConf.setDebug((byte) 1);
        }
