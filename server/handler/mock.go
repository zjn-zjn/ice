...
func (h *MockHandler) execute(w http.ResponseWriter, r *http.Request) (any, error) {
	...
	if req.App <= 0 {
		return nil, model.InputError("app is required")
	}
	if req.IceId <= 0 && req.ConfId <= 0 && req.Scene == "" {
		return nil, model.InputError("one of iceId, confId, scene is required")
	}
...
func (h *MockHandler) schema(w http.ResponseWriter, r *http.Request) (any, error) {
	...
	if iceId <= 0 && confId <= 0 {
		return nil, model.InputError("one of iceId or confId is required")
	}
